...
def main(args):
    ...
    for epoch in range(0,args.epochs):
        ...
        print("Baseline loss for epoch",epoch,"=",loss.item())
        print("Baseline accuracy for epoch",epoch,"=",acc/j)
    ...
    print("final training loss for Baseline = ",loss)
    print("final training acc for Baseline = ",acc/j)
    print("final validation loss for Baseline = ",val_loss)
    print("final validation accuracy for Baseline = ",val_acc)
    print("final test loss for Baseline = ",test_loss)
    print("final test accuracy for Baseline = ",test_acc)
    ...
    '''
    model = CNN(args.emb_dim,vocab,50,[1,1])
    optimizer = optim.Adam(model.parameters(),lr=args.lr)
    loss_fnc = nn.BCEWithLogitsLoss()
    ...
    for epoch in range(0,args.epochs):
        ...
        losses.append(loss)
        accs.append(acc/j)
        print("CNN loss for epoch",epoch,"=",loss.item())
        print("CNN accuracy for epoch",epoch,"=",acc/j)
    ...
    plt.plot(epochs,accs,label="Training")
    plt.plot(epochs,accs_val,label="Validation")
    plt.plot(epochs,accs_test,label="Test")
    plt.title('Accuracy vs. epoch')
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.legend()
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(['64','0.001','100','baseline','100','100','50','6'])
    main(args)
